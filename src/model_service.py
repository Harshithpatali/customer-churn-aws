...
def predict_one(payload: dict):
    ...
    monthly = float(payload.get("MonthlyCharges", 0) or 0)
    return {
        "churn_probability": p,
        "risk_band": risk_band(p, threshold),
        "prediction": int(p >= threshold),
        "threshold": threshold,
        "estimated_monthly_revenue_at_risk": round(p * monthly, 2),
        "recommendation": recommendation(payload, p),
    }
