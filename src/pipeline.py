...
from .config import CATEGORICAL_FEATURES, FEATURES_PATH, ID_COL, MODEL_PATH, NUMERIC_FEATURES, RANDOM_STATE, TARGET
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    out["service_count"] = (out[["PhoneService", "OnlineSecurity", "OnlineBackup", "DeviceProtection", "TechSupport", "StreamingTV", "StreamingMovies"]] == "Yes").sum(axis=1)
...
def build_preprocessor() -> ColumnTransformer:
    numeric = NUMERIC_FEATURES + ["service_count", "monthly_to_tenure_value"]
...
def optimize_threshold(y, prob):
    ...
    for threshold in np.arange(0.20, 0.81, 0.01):
        ...
        # Favor recall while retaining meaningful precision.
...
def train(csv_path: Path = None, output_dir: Path = None):
    ...
    baseline = make_pipeline(LogisticRegression(max_iter=3000, class_weight="balanced", random_state=RANDOM_STATE))
    rf = make_pipeline(__import__("sklearn.ensemble", fromlist=["RandomForestClassifier"]).RandomForestClassifier(n_estimators=400, class_weight="balanced", random_state=RANDOM_STATE, n_jobs=-1))
    xgb = make_pipeline(XGBClassifier(
        n_estimators=500, max_depth=4, learning_rate=0.035, subsample=0.85,
        colsample_bytree=0.85, reg_lambda=2.0, min_child_weight=3,
        objective="binary:logistic", eval_metric="logloss", random_state=RANDOM_STATE,
        n_jobs=4,
    ))

    candidates = {"logistic_regression": baseline, "random_forest": rf, "xgboost": xgb}
    comparison = {}
    for name, model in candidates.items():
        model.fit(X_train, y_train)
        comparison[name] = metrics(y_test, model.predict_proba(X_test)[:, 1])

    # Tune only the strongest tree family to keep training practical.
    search = RandomizedSearchCV(
        xgb,
        {"model__max_depth": [3, 4, 5], "model__learning_rate": [0.025, 0.035, 0.05],
         "model__subsample": [0.75, 0.85, 1.0], "model__colsample_bytree": [0.75, 0.85, 1.0],
         "model__min_child_weight": [1, 3, 5]},
        n_iter=12, scoring="average_precision", cv=StratifiedKFold(3, shuffle=True, random_state=RANDOM_STATE),
    ...
    report = {"dataset_rows": int(len(df)), "churn_rate": float(y.mean()), "comparison": comparison, "best_params": search.best_params_, "test": final_metrics, "threshold_selection": threshold}
